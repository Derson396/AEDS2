...
public class Nove {
     ...
     * Game temp = Jogos.get(0);
     * Jogos.set(0, Jogos.get(i));
     * Jogos.set(i, temp);
     * 
     ...
     * if (l < n &&
     * Jogos.get(l).getReleaseDate().after(Jogos.get(largest).getReleaseDate()))
     * largest = l;
     * 
     * // If right child is larger than largest so far
     * if (r < n &&
     * Jogos.get(r).getReleaseDate().after(Jogos.get(largest).getReleaseDate()))
     * largest = r;
     * 
     * // If largest is not root
     * if (largest != i) {
     * Game swap = Jogos.get(i);
     * Jogos.set(i, Jogos.get(largest));
     * Jogos.set(largest, swap);
     * 
     ...
     * }
     * 
     * // methodo para organizar quicksort
     * private static void quickSort(int inicio, int fim) {
     * if (inicio < fim) {
     ...
     * }
     * 
     * private static int separar(int inicio, int fim) {
     * Game pivo = Jogos.get(inicio);
     * int i = inicio + 1, f = fim;
     * while (i <= f) {
     * if (Jogos.get(i).getReleaseDate().before(pivo.getReleaseDate())
     * || Jogos.get(i).getReleaseDate().equals(pivo.getReleaseDate()))
     * i++;
     * else if (pivo.getReleaseDate().before(Jogos.get(f).getReleaseDate()))
     * f--;
     * else {
     * Game troca = Jogos.get(i);
     * Jogos.set(i, Jogos.get(f));
     * Jogos.set(f, troca);
     * i++;
     * f--;
     * }
     * }
     * Jogos.set(inicio, Jogos.get(f));
     * Jogos.set(f, pivo);
     * return f;
     * }
     * 
     * //methodo organizador bolha
     ...
     * if (Jogos.get(i).getDevelopers().compareTo(Jogos.get(i+1).getDevelopers())>0)
     * {
     * aux = Jogos.get(i);
     ...
     * }
     */
    //method para organizar merge
    public static void mergeSort(ArrayList<Game> a, int n) {
        if (n < 2) {
            return;
        }
        int mid = n / 2;
    ...
    public static void merge(
            ArrayList<Game> a, ArrayList<Game> l, ArrayList<Game> r, int left, int right) {

        int i = 0, j = 0, k = 0;
        while (i < left && j < right) {
            if (l.get(i).getUpvotes()<=r.get(j).getUpvotes()) {
                a.set(k++,l.get(i++));
            } else {
                a.set(k++,r.get(j++));
            }
    ...
    public static void main(String args[]) {
        ...
        // insertion_sort();
        // HeapSort();
        // quickSort(0, Jogos.size() - 1);
        //bubbleSort();
        mergeSort(Jogos, Jogos.size());
