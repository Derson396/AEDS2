...
class SteamQ4 {
    ...
    public SteamQ4(int app_id, int age, int dlcs, int avg_pt, float price, float upvotes, String name, String owners,
    ...
    public SteamQ4 clone(SteamQ4 clone) {
        return (new SteamQ4(clone.age, clone.app_id, clone.avg_pt, clone.dlcs, clone.price, clone.upvotes,
    ...
    public static String pegaNome(String Linha) {
        int n = 0;
        int loc = 0, i;
        for (i = 0; n < 2; i++) {
            if (Linha.charAt(i) == ',') {
        ...
        String resp = Linha.substring(loc + 1, i - 1);
        return resp;
    }

    public static boolean binarySearch(String[] nomes, String entrada) {
        int left = 0, right = nomes.length - 1;
        while (left <= right) {
            int mid = left + (right - left) / 2;

            if (nomes[mid].equals(entrada)) {
                return true;
            }

            if ((entrada.compareTo(nomes[mid])) > 0) {
                left = mid + 1;
            }
