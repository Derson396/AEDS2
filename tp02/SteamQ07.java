...
/**
 * Lista estatica (author Max do Val Machado) adaptada para a classe Game
 * LISTA
 */
class Lista {
    private Game[] array;
    ...
    public boolean pesqSeq(int[] vet, int x) {
        boolean resp = false;
        int n = vet.length;
    ...
    public boolean pesquisaBinaria(String x) {
        boolean resp = false;
        int dir = (this.n - 1), esq = 0, meio;

        while (esq <= dir) {
            meio = (esq + dir) / 2;
            if (array[meio].getName().equals(x)) {
                resp = true;
                esq = dir + 1;
            } else if (array[meio].getName().compareTo(x) < 0) { // (x > vet[meio])
                esq = meio + 1;
            } else {
                dir = meio - 1;
            }
...
/**
 * Lista estatica (author Max do Val Machado) adaptada para PILHA a classe Game
 * PILHA
 */
class Pilha {
    private Game[] array;
     ...
     * IF
     */
    public void empilhar(Game x) throws Exception {
...
/**
 * Fila estatica (author Max do Val Machado) adaptada para a classe Game
 */
class Fila {
    private Game[] array;
...
public class SteamQ07 {

    static Fila filaGames = new Fila(5);

    public static boolean isFim(String s) {
        return (s.length() == 3 && s.charAt(0) == 'F' && s.charAt(1) == 'I' && s.charAt(2) == 'M');
    }
/**
 * Função principal para ler do arquivo e chamar as funçoes de Fila
 * @param args
 * @throws Exception
 */
    public static void main(String args[]) throws Exception {
        ...
        comandsLenght = Integer.parseInt(MyIO.readLine());
        do {
            lineCount++;
            count02++;
            input02[count02] = MyIO.readLine();
            comandsLenght--;
        } while (comandsLenght > 0);

        for (int i = 0; i < count01; i++) {
            Game jogo = new Game();
            jogo.readClass(input01[i], "/tmp/games.csv");
            filaGames.inserir(jogo);
            somaAvgPts = 0;
            GamesLenght = 0;
            for (int j = filaGames.getPrimeiro(); j != filaGames
                    .getUltimo(); j = ((j + 1) % filaGames.getArraySize())) {
                somaAvgPts += filaGames.getGame(j).getAvgPt();
                GamesLenght++;
            }
            media = somaAvgPts / GamesLenght;
        ...
        for (int i = 0; i < count02 + 1; i++) {
            ...
            if (input02[i].split(" ")[0].compareTo("I") == 0) {
                jogo.readClass(input02[i].split(" ")[1], "/tmp/games.csv");
                filaGames.inserir(jogo);
            ...
            } else if (input02[i].split(" ")[0].compareTo("R") == 0) {
                jogo = filaGames.remover();
                System.out.print("R\n(R) " + jogo.getName() + "\n");
