...
char* getName (game *jogo){
    return jogo->name.buf;
}
...
/**
 * Mostra os elementos da pilha separados por espacos de baixo para cima
 */
int cont_mostrar2;
void mostrar2b(Celula* i) {
    if (i != NULL) {
        mostrar2b(i->prox);
        game_print(&(i->elemento), cont_mostrar2);
        cont_mostrar2++;
    }
    if (i == NULL) {
        cont_mostrar2 = 0;
...
int main(int argc, char** argv)
{
    ...
    fgets(input, MAX_LINE_SIZE, stdin);
    input[strlen(input) - 1] = '\0';
    quantComandos = atoi(input);
    //printf("QUANT COMANDOS: %d\n", quantComandos);
    ...
    game_read_csv(games, entrada1, cont1);
    ...
    for ( i = 0; i < cont2+1; i++) {
        ...
        if (e1 == 'I'){
            tam = strlen(entrada2[i]);
            comandID = substr(entrada2[i], 2, tam);
            comandIdInt = atoi(comandID);
            id_unico[0] = comandIdInt;

            game_read_csv(uniqueGame, id_unico, 1);
            inserir(*uniqueGame);
        }
